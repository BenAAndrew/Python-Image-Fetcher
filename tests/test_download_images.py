...
from tests.helpers import reset_directory, TestConfig


class TestDownloadImages:
    def test_download_image(self):
        reset_directory(TestConfig.IMAGE_DIRECTORY)
        url = TestConfig.URLS[0]
    ...
    def test_download_image_with_timeout_within_time(self):
        ...
        successful = download_image_with_timeout(url, timeout, TestConfig.IMAGE_DIRECTORY, TestConfig.HEADERS)
    ...
    def test_multi_thread_image_download(self):
        ...
        assert total_images == len(TestConfig.URLS)
        for url in TestConfig.URLS:
            file_name = escape_image_name(url)
            assert file_name in os.listdir(TestConfig.IMAGE_DIRECTORY)
            assert os.path.getsize(os.path.join(TestConfig.IMAGE_DIRECTORY, file_name)) > 0
    ...
    def teardown_class(cls):
        rmtree(TestConfig.IMAGE_DIRECTORY)
